  ...
  namespace('migration', function () {
    ...
    var createMigration = function (name, options) {
        var opts = options || {}
          ...
          , text = fs.readFileSync(path.join(genDirname, 'template',
            'migration.js.ejs'), 'utf8').toString()
    ...
    task('run', {async: true}, function (targetMigration) {
      ...
      var targetTimestampString = targetMigration ?
        ...
        , finderTask = jake.Task['model:migration:filterMigrations']
        , runnerTask = jake.Task['model:migration:runMigrations'];
